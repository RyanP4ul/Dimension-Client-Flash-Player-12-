    ...
    public class LPFFrameQtySelector extends LPFFrame 
    {
        ...
        protected function fDraw():void
        {
            if (fData != null)
            {
                ...
                {
                    _val = (_min = 1);
                    _max = game.world.maximumShopSells(fData);
                }
                else
                {
                    _val = (_min = fData.iQty);
                    _max = game.world.maximumShopBuys(fData);
                }
                val;
                updateValues();
                updateHandle();
        ...
        override public function notify(_arg_1:Object):void
        {
            if (_arg_1.eventType == "listItemASel")
            {
                ...
                {
                    if (game.ui.mcPopup.currentLabel == "EnhShop")
                    {
                        return;
                    }

                    if ((getLayout().sMode == "shopSell" || getLayout().sMode == "shopBuy") && (game.world.maximumShopBuys(_arg_1.fData.iSel) > 1 || game.world.maximumShopSells(_arg_1.fData.iSel) > 1))
                    {
