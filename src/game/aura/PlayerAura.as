...
public class PlayerAura extends MovieClip {
    ...
    public function PlayerAura(game:Game) {
        this.game = game;
...
//        if (!game.world.myAvatar.target) return;
        ...
        {
            trace("CREATE ICON MC => ASSET => " + asset);

            assetClass = asset && asset != "undefined"
                    ? asset.indexOf(",") > -1
                            ? game.world.getClass(asset.split(",")[asset.split(",").length - 1])
                            : game.world.getClass(asset) as Class : game.world.getClass("isp2") as Class;
...
//            mouseChildren = mouseEnabled = false;
    ...
    {
        if (!game.ui) return;
        game.onRemoveChildrens([auraContainer]);
        icons = {};
        iconPriority = [];
    }
    ...
    public function handleAura(_arg_1:Object):*
    {
        ...
        for each (_local_4 in _arg_1.a)
        {
            ...
            {
                if (_local_4.tInf.substring(2) == game.net.myUserId)
                {
                    if (_local_4.auras)
                    {
                        for each (_local_5 in _local_4.auras)
                        {
                            if (_local_4.cmd.indexOf("+") > -1)
                            {
                                ...
                                else
                                {
                                    ...
                                    for each (_local_6 in game.world.myAvatar.dataLeaf.auras)
                                    {
                    ...
                    else
                    {
                        if (_local_4.cmd.indexOf("+") > -1)
                        {
                            ...
                            else
                            {
                                ...
                                for each (_local_7 in game.world.myAvatar.dataLeaf.auras)
                                {
